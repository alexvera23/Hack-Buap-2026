...
export default function AnimalCareIndicators() {
  // Datos zoosanitarios históricos
  const zooHealthHistory = [
    { date: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), score: 72 },
    { date: new Date(Date.now() - 25 * 24 * 60 * 60 * 1000), score: 75 },
    { date: new Date(Date.now() - 20 * 24 * 60 * 60 * 1000), score: 78 },
    { date: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000), score: 82 },
    { date: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000), score: 85 },
    { date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000), score: 88 },
    { date: new Date(Date.now()), score: 92 },
  ];

  const generateChartData = () => {
    return zooHealthHistory.map((item) => ({
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-4xl font-bold text-gray-900">Indicadores Zoosanitarios</h1>
          </div>
          <p className="text-gray-600">Análisis histórico y tendencias de bioseguridad animal y productividad</p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-amber-600 mt-2">{maxScore}%</p>
              </div>
              <div className="text-amber-700 bg-amber-100 rounded-full p-2">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Basado en registros de bioseguridad</p>
              </div>
        ...
        <div className="bg-white rounded-lg shadow p-8 mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Evolución de Puntuación Zoosanitaria</h2>
              <p className="text-sm text-gray-500">Gráfica estática con el seguimiento de la bioseguridad animal en el tiempo.</p>
            </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-gray-50 p-6">
            <div className="overflow-x-auto">
              <svg viewBox="0 0 720 260" className="w-full h-80">
                ...
                <polyline fill="url(#lineGradient)" stroke="#D97706" strokeWidth={4} points={chartPoints} opacity={0.8} />
        ...
        <div className="bg-white rounded-lg shadow p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Impacto en Producción y Salud Animal</h2>
            ...
            {productionMetrics.map((metric) => (
              <div key={metric.id} className="border rounded-lg p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-6 text-sm">
                    <span className="text-gray-600">
                      <span className="font-bold text-gray-900">{metric.animalHealth}%</span> Salud Animal
                    </span>
                    <span className="text-gray-600">
                      <span className="font-bold text-gray-900">{metric.mortality}%</span> Mortalidad
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-gray-600 mb-2">Índice de Salud Animal</p>
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-3 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-gradient-to-r from-green-400 to-green-600"
                          style={{ width: `${metric.animalHealth}%` }}
                        ></div>
                      </div>
                      <span className="font-bold text-gray-900">{metric.animalHealth}%</span>
                    </div>
                  </div>

                  <div>
                    <p className="text-sm text-gray-600 mb-2">Índice de Productividad</p>
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-3 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-gradient-to-r from-blue-400 to-blue-600"
                          style={{ width: `${metric.productionIndex * 100}%` }}
                        ></div>
                      </div>
                      <span className="font-bold text-gray-900">{(metric.productionIndex * 100).toFixed(0)}%</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-blue-50 rounded-lg p-6 border border-blue-200">
            ...
            <p className="text-sm text-blue-800">
              Tu puntuación zoosanitaria ha mejorado un <strong>20%</strong> en las últimas 4 semanas.
              La salud animal y la productividad han mostrado tendencias positivas consistentes.
            </p>
          </div>
          ...
          <div className="bg-amber-50 rounded-lg p-6 border border-amber-200">
            ...
            <p className="text-sm text-amber-800">
              Se recomienda intensificar el monitoreo de vectores en la próxima temporada de lluvias.
              Mantener la vigilancia predictiva y considerar refuerzos en protocolos de desinfección.
            </p>
          </div>
        </div>
      </div>
